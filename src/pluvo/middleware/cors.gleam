...
fn check_access_control_request(
  ctx: Context,
  origin,
  allowed_headers: String,
) -> Response {
  use acr_headers <- context.then(
    ctx
    |> context.get_header("Access-Control-Request-Headers"),
  )
  use acr_method <- context.then(
    ctx
    |> context.get_header("Access-Control-Request-Method"),
  )
